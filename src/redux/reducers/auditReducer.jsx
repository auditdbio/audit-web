import {
    AUDIT_REQUEST_CREATE, CLEAR_MESSAGES, CONFIRM_AUDIT, DELETE_AUDIT, DELETE_REQUEST,
    GET_AUDIT_REQUEST, GET_AUDITS, REQUEST_ERROR, SUBMIT_AUDIT
...
export const auditReducer = (state = initialState, action) => {
    ...
    switch (action.type) {
        ...
        case DELETE_AUDIT:
            return {...state, audits: state.audits.filter(request => request.id !== action.payload.id)}
        case GET_AUDITS:
